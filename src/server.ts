...
import { getProfile, updateProfile, Profile, Demographics, getAllWaIds } from './store';
...
const normalize = (s = '') =>
    s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^\w+ ]+/g, '').replace(/\s+/g, ' ').trim();
...
type State =
    ...
    | 'menu'
    | 'ask_cabildo_name'
    | 'demographics_0' | 'demographics_1' | 'demographics_2' | 'demographics_3' | 'demographics_4' | 'demographics_5'
    | 'station_menu'
    | 'station_input'
    | 'after_station'
...
function pickFromOptions(input: string, options: string[]): string | null {
    ...
    if (alt.includes(normInput)) return options.find(o => normalize(o) === normalize('Prefiero no contestar')) || options[options.length - 1];
...
app.post('/webhook', async (req: any, res: Response) => {
    ...
    if (s.stationsDone.length === 0 && (profile.stationsDone?.length || 0) > 0) s.stationsDone = [...(profile.stationsDone as number[])];
    ...
    switch (s.state) {
        ...
        case 'ask_cabildo_name': {
            ...
            await updateProfile(from, p => { p.lastCabildoName = s.cabildoName; });
        ...
        case 'demographics_5': {
            ...
            await updateProfile(from, p => { (p.demographics as any)[demo.key as keyof Demographics] = picked; });
            const next = idx + 1;
            if (next < DEMOS.length) {
                s.state = `demographics_${next}` as State;
                await sendText(from, [DEMOS[next].q]);
            } else {
                await updateProfile(from, p => { p.demographicsCompleted = true; });
                s.state = 'station_menu';
                await sendText(from, stationMenu(remainingStationsUnion(s, profile)));
            }
            break;
        ...
        case 'post_all_stations_phrase': {
            ...
            await updateProfile(from, p => { p.finalWord = word; });
            s.state = 'consent';
